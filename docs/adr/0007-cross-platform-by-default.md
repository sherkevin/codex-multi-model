...
3. **上游特性一律配置化，不写死。** 字节硬顶、token 硬顶、三组错误签名都做成
   ...
   （字节默认 4.5MiB：多降一档图只是画质损失，漏设则 TooLarge → 死锁，见 0004）。
   默认值本身必须**实测**得到，不能抄上游错误消息里的数字，也不能假设它按我们
   发送的字节计量 —— 见 [0009](0009-upstream-byte-limit-is-escaped-and-undersold.md)。
   所以配套给了 `probe-body-byte-limit.py`，换网关时重新测一遍即可。
